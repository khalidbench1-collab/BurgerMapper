...
- Standing exception granted by the user on 2026-07-18: during Phase 5, Codex may run exactly one live `gpt-5.6-luna` Responses API smoke request without asking again, but only after confirming all of these bounds: use a short trusted fictional sample or synthetic goal as `input_text`; send no file, private data, source-research query, or personal information; use `low` reasoning and no more than 2,000 output tokens; make no retry, verification pass, tone-polishing call, tool call, or second request; set `store: false`; and confirm from current official pricing that the estimated billable cost is no more than USD 0.10. Record only pass/fail, model ID, date, latency, token usage, and cost metadata—never the prompt or response. If any bound cannot be confirmed or the call fails, stop the live test and report it without another call. This exception does not authorize later development, research, evaluation, or production API traffic.
