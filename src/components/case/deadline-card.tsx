...
export function DeadlineCard({ analysis }: { analysis: CaseAnalysis }) {
  ...
  return (
    <section aria-labelledby="deadline-heading" className="rounded-2xl border border-[#e2c7b7] bg-[#fff8f3] p-5 sm:p-6">
      ...
      {analysis.detectedDeadline && analysis.deadlineProvenance?.confirmationRequired ? (
        <p className="mt-3 text-xs leading-5 text-[#6b554a]">
          {analysis.outputLanguage === "de"
            ? "Dokumentangabe: Prüfen Sie dieses Datum an Ihrem Originaldokument; es wurde nicht durch eine offizielle Quelle bestätigt."
            : analysis.outputLanguage === "ar"
              ? "حقيقة من المستند: أكّد هذا التاريخ من وثيقتك الأصلية؛ لم يتم التحقق منه بواسطة مصدر رسمي."
              : "Document fact: confirm this date against your original document; it has not been verified by an official source."}
        </p>
      ) : null}

      <div className="mt-5 border-t border-[#ead5ca] pt-5">
        <p className="text-sm font-semibold text-[#5e4539]">{copy.openPoints}</p>
